...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EGulden: starting difficulty is 1 / 2^12
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EGulden: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // EGulden: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // E-Gulden: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EGuldenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EGuldenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EGuldenMiner(CWallet *pwallet)
{
    printf("EGuldenMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("e-gulden-miner");
    ...
    try { loop {
        ...
        printf("Running EGulden with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EGuldenMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EGuldenMiner, pwallet));
}
